...
__global__ void gemm_demo4(const float *dA, const float *dB, float *dC, int M, int N, int K) {
...
static void gemm_hand(float *dC, const float *dA, const float *dB, int M, int N, int K) {
    ...
    gemm_demo4<16, STRIDE><<<grid, block, shared_size>>>(dA, dB, dC, M, N, K);
...
// ======= 一行注册（你想要的“宏包裹”） =======
REGISTER_OP_FUNCS("gemm_demo4", correctness, perf);
