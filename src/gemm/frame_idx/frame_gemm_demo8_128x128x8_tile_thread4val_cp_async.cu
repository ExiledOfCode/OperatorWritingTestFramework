...
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
    asm volatile("cp.async.ca.shared.global [%0], [%1], %2;\n" ::"r"(cvta_to_shared_u32(shared_ptr)), "l"(global_ptr), "n"(BYTES));
#else
    (void)shared_ptr;
    (void)global_ptr;
#endif
...
__global__ void frame_gemm_demo8_128x128x8_tile_thread4val_cp_async(float *dA, float *dB, float *dC, int M, int N, int K) {
...
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
    // shared_A 当前布局是 [k][m]，4 个元素在 shared 里不是连续地址，所以这里保留原有映射，拆成 4 次 4B cp.async。
    cp_async_ca<4>(&shared_A[0][shared_A_col + 0][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, shared_A_col + 0)));
    cp_async_ca<4>(&shared_A[0][shared_A_col + 1][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, shared_A_col + 1)));
    cp_async_ca<4>(&shared_A[0][shared_A_col + 2][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, shared_A_col + 2)));
    cp_async_ca<4>(&shared_A[0][shared_A_col + 3][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, shared_A_col + 3)));

    // shared_B 的最后一维连续，保留原来的 float4 线程映射，直接做一次 16B cp.async。
    cp_async_ca<16>(&shared_B[0][shared_B_row][shared_B_col], &(B(shared_B_row, block_tile_C_start_x + shared_B_col)));
...
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
        cp_async_ca<4>(&shared_A[cnt ^ 1][shared_A_col + 0][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, k_base + shared_A_col + 0)));
        cp_async_ca<4>(&shared_A[cnt ^ 1][shared_A_col + 1][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, k_base + shared_A_col + 1)));
        cp_async_ca<4>(&shared_A[cnt ^ 1][shared_A_col + 2][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, k_base + shared_A_col + 2)));
        cp_async_ca<4>(&shared_A[cnt ^ 1][shared_A_col + 3][shared_A_row], &(A(block_tile_C_start_y + shared_A_row, k_base + shared_A_col + 3)));

        cp_async_ca<16>(&shared_B[cnt ^ 1][shared_B_row][shared_B_col], &(B(k_base + shared_B_row, block_tile_C_start_x + shared_B_col)));
        cp_async_commit_group();
#else
...
static void gemm_hand(float *dC, float *dA, float *dB, int M, int N, int K) {
    ...
    frame_gemm_demo8_128x128x8_tile_thread4val_cp_async<BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N, BLOCK_TILE_SIZE_K, THREAD_SIZE_X, THREAD_SIZE_Y>
...
LC_REGISTER_GEMM_OP("frame_gemm_demo8_128x128x8_tile_thread4val_cp_async", opfw::cpu_gemm_f32, gemm_launch);
