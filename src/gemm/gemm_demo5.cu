...
__global__ void gemm_demo5(const float *dA, const float *dB, float *dC, int M, int N, int K) {
        ...
        for (int s = 0; s < K / TILE; s++) {
            ...
            for (int y_stride = 0; y_stride < STRIDE; y_stride++) {
                for (int x_stride = 0; x_stride < STRIDE; x_stride++) {
                    int A_shared_idx = (ty + y_stride * BLOCK_SIZE) * TILE + tx + x_stride * BLOCK_SIZE;
                    int B_shared_idx = (ty + y_stride * BLOCK_SIZE) * TILE + tx + x_stride * BLOCK_SIZE;
...
static void gemm_hand(float *dC, const float *dA, const float *dB, int M, int N, int K) {
    ...
    gemm_demo5<16, STRIDE><<<grid, block, shared_size>>>(dA, dB, dC, M, N, K);
...
// ======= 一行注册（你想要的“宏包裹”） =======
REGISTER_OP_FUNCS("gemm_demo5", correctness, perf);
