...
// =======================
__global__ void reduce_demo6(const float *dA, float *dY, int M, int K) {
    int tx = threadIdx.x;
    int bx = blockIdx.x;
...
static void reduce_hand(float *dY, const float *dA, int M, int K) {
    ...
    reduce_demo6<<<grid, block>>>(dA, dY, M, K);
    CUDA_CHECK(cudaGetLastError());
}

LC_REGISTER_REDUCE_ROW_OP("reduce_demo6", reduce_cpu_row_sum, reduce_hand);